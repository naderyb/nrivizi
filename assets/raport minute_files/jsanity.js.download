if (typeof jSanity !== "undefined")
  throw "jSanity has been defined, please check if there's any duplicate reference.";
...
(function (ns) {
  ...
  var defaults = {
    ...
    // Callback to handle URLs referencing external content
    externalContentCallback:
  ...
  var jSanityClass = function (schedulerClass, options) {
        ...
        switch (child.nodeType) {
          case child.ELEMENT_NODE:
            ...
            if (tagN in knownHTML[ns].knownElements) {
              ...
              if (
                !itemOptions.allowAudioVideo &&
                (tagN === "video" || tagN === "audio" || tagN === "source")
              ) {
                killElt = true;
              }
              ...
              if (!killElt) {
                ...
                for (var i = 0; i < child.attributes.length; i++) {
                  ...
                  if (attN in knownHTML[ns].knownAttributes) {
                    ...
                    if (attN === "href") {
                      ...
                      if (itemOptions.allowLinks) {
                        for (var protocol in knownProtocols) {
                          ...
                          if (
                            child.attributes[i].value.substring(
                              0,
                              protocol.length
                            ) === protocol
                          ) {
                      ...
                      if (child.attributes[i].value.length > 0) {
                        if (
                          child.attributes[i].value.length >
                            fullPrefix.length &&
                          child.attributes[i].value.substring(
                            0,
                            fullPrefix.length
                          ...
                          cElt.setAttribute(
                            child.attributes[i].name,
                            child.attributes[i].value
                          );
                        } else {
                          cElt.setAttribute(
                            child.attributes[i].name,
                            fullPrefix + child.attributes[i].value
                          );
                        }
                      }
                    } else if (attN === "src") {
                      ...
                      if (itemOptions.externalContentCallback !== null) {
                        output = itemOptions.externalContentCallback(
                          "attribute",
                          "src",
                          output,
                          knownProtocols
                    ...
                    itemOptions.dataAttributeCallback !== null &&
                    attN.substring(0, 5) === "data-"
                  ) {
                    ...
                    outAttribute = itemOptions.dataAttributeCallback(
                      child.attributes[i].name,
                      child.attributes[i].value
                    );
                ...
                for (var i = 0; i < child.style.length; i++) {
                  ...
                  if (childStyle in knownCSSProperties) {
                    ...
                    try {
                      ...
                      consoleLog(
                        0,
                        "Unable to set CSS property: " + childStyle
                      );
                    }
                  } else {
                    if (itemOptions.directModifySource) {
                    ...
                    consoleLog(
                      1,
                      "Encountered unsupported style property: " + childStyle
                    );
                    if (itemOptions.unsupportedContentCallback) {
                      itemOptions.unsupportedContentCallback(
                        "style",
                        childStyle
                      );
                    }
    ...
    this.sanitizeMethod = function sanitizeMethod(options) {
      ...
      scheduleNewWalkerJob(
        tw.currentNode,
        "default",
        itemOptions.isolatedTargetDOM ? destDoc.body : spanBuffer
      );
